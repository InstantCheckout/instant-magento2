define([
    ...
    window.onmessage = function (e) {
        ...
        switch (dataObj.type) {
            ...
            case 'mergeCarts': {
                const { fromCartId, toCartId } = dataObj.body;
                $.ajax({
                    type: 'POST',
                    url: window.location.origin + "/instant/cart/merge",
                    data: { fromCartId, toCartId },
                    dataType: 'json',
                    retryLimit: 3,
                    error: function () {
                        this.showErrorAlert();
    ...
    const init = function () {
        ...
        return window.open('http://' + instantUrl, '', 'location=yes,height=' + windowHeight + ',width=' + windowWidth + ',top=' + posY + ',left=' + posX + ',scrollbars=yes,status=yes');
    };
    ...
    const canSetWindowLocation = function () {
    ...
    window.Instant['getCheckoutUrl'] = getCheckoutUrl;
    window.Instant['init'] = init;
    window.Instant['isClientMobileOrTablet'] = isClientMobileOrTablet;
    window.Instant['canSetWindowLocation'] = canSetWindowLocation;
});
